...
飞船:%s]], GUserSpaceship.Info.Position.X, GUserSpaceship.Info.Position.Y, GUserSpaceship.Info.Name))
    ...
    local planetId = self.ScreenPlanets[PointToStr(self.CursorScreenPosition)]
...
function _Radar.GetPlanetOnScreenByPosition(self, position)
    return GWorld:GetPlanetByPlanetId(self.ScreenPlanets[PointToStr(self:PositionToScreenPosition(position))])
end

-- 根据屏幕坐标系获取屏幕上的星球
function _Radar.GetPlanetOnScreenByScreenPosition(self, position)
    return GWorld:GetPlanetByPlanetId(self.ScreenPlanets[PointToStr(position)])
end
...
function _Radar.RefreshScreenPlanets(self, planets, rectangle)
    ...
    for _, planet in pairs(planets) do
        ...
        self.ScreenPlanets[PointToStr(planet.ScreenPosition)] = planet.Info.PlanetId
    end
end
